...
def get_view_crop_with_scrollbars(img):
    ...
    # Calculate max possible pan values
    ...
    # Bottom-right corner intersection box
    cv2.rectangle(
        canvas, (bar_v_x1, bar_h_y1), (img_w, img_h), (30, 30, 30), -1
    )
...
def draw_mouse_bbox(event, x, y, flags, param):
    ...
    # Check if interaction is over embedded scrollbars
...
def run_interactive_annotator():
    ...
    print("=" * 50)
    print("• Left Scrollbar    : Drag horizontal scrollbar at bottom edge")
    print("• Right Scrollbar   : Drag vertical scrollbar at right edge")
    print("• Press '+' / '-'   : Zoom In / Zoom Out")
    print("• Arrows / WASD     : Pan around zoomed image")
    ...
    LEFT_KEYS = {2424832, 81, 65361, 0x250000, 37}
    UP_KEYS = {2490368, 82, 65362, 0x260000, 38}
    RIGHT_KEYS = {2555904, 83, 65363, 0x270000, 39}
    DOWN_KEYS = {2621440, 84, 65364, 0x280000, 40}
    ...
    while True:
        ...
        if full_key == -1 and char_key == 255:
            continue
        ...
        # Navigation / Panning (Arrow Keys + WASD)
        elif full_key in LEFT_KEYS or char_key in (ord("a"), ord("A")):
            pan_x -= pan_step
            clamp_pan()
        elif full_key in RIGHT_KEYS or char_key in (ord("d"), ord("D")):
            pan_x += pan_step
            clamp_pan()
        elif full_key in UP_KEYS or char_key in (ord("w"), ord("W")):
            pan_y -= pan_step
            clamp_pan()
        elif full_key in DOWN_KEYS or char_key in (ord("s"), ord("S")):
            pan_y += pan_step
            clamp_pan()
